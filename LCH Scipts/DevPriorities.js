...
async function main() {
	...
	let LCU = processTickets(results[0]);
	let LCE = processTickets(results[1]);
	let LCD = processTickets(results[2]);

	writeTickets(LCU, LCE, LCD);
}

async function fetchTickets() {
	const promises = [];

	promises.push(jira.searchJira(`${LCUTickets} and ${issueLink}`, {maxResults:2000}));
	promises.push(jira.searchJira(`${LCETickets} and ${issueLink}`, {maxResults:2000}));
	promises.push(jira.searchJira(`${LCDTickets} and ${issueLink}`, {maxResults:2000}));
